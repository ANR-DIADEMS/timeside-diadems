...
def temporalmodeling(desc):
    ...
    field_name    = []
    param_val    = np.zeros(NB_DESC, float)
    index = 0
    ...
    for i in xrange(0, len(desc)):
        for j in xrange(0, len(desc[i]._fields)):
            ...
            if i == 0 and j < len(desc[i]._fields)-1:    
                ...
                field_v = np.array([desc[i][j]])
            else:    ## compute median / IRQ
                if desc[i]._fields[j] == "AutoCorr":
                    field_n = []
                    field_v = np.zeros(config_s.xcorr_nb_coeff * 2, float)
...
def timbre_desc_time(s, Fs, N, rec):
    step = round(N/rec)
    nb_trame    = int(np.ceil( len(s) / step))
    T_NOISE        = -60 #-np.inf
    t            = np.zeros(nb_trame,float)
    desc_struc    = np.zeros((nb_trame, NB_DESC), float)
...
def ERBpower(a,Fs, hopsize, bwfactor=1.):
    ...
    m        = len(a)
    offset    = np.round( mt.centroid( pow(window, 2.)) )
    a        = np.concatenate( (np.zeros(offset), np.squeeze(a), np.zeros(wsize-offset)) )
...
def FHarrep(f_Sig_v, Fs):
    ...
    L_n                = np.round(L_sec*Fs)
    STEP_n            = np.round(STEP_sec*Fs)
    ...
    totalenergy_3m    = np.zeros( (nb_frame, len(lag_f0_hz_v), len(inharmo_coef_v)), float)
    stock_pos_4m    = np.zeros( (nb_frame, len(lag_f0_hz_v), len(inharmo_coef_v), config_s.nb_harmo), int)
    
    f0_hz_v = Fevalbp(f0_bp, T_v)
    # === candidate_f0_hz_m (nb_frame, nb_delta)    
    candidate_f0_hz_m    = np.repeat(np.array([f0_hz_v,]).T, nb_delta, axis=1) + np.repeat([lag_f0_hz_v,], nb_frame, axis=0)
    ...
    for num_inharmo in xrange(0, nb_inharmo):
        ...
        for num_delta in xrange(0, nb_delta):
            ...
            for num_frame in xrange(0, nb_frame):
                totalenergy_3m[num_frame, num_delta, num_inharmo]    = np.sum( B_m[ candidate_harmo_pos_m[num_frame, :], num_frame] )
    
    # === choix du coefficient d'inharmonicite
    score_v = np.zeros( nb_inharmo, float)
    for num_inharmo in xrange(0, nb_inharmo):
        score_v[num_inharmo] = np.sum( np.max( np.squeeze( totalenergy_3m[:, :, num_inharmo]), axis=0) )
    ...
    if calcul > 0.01:
        ...
        num_inharmo = 1
    totalenergy_2m    = np.squeeze( totalenergy_3m[:, :, num_inharmo] )

    PartTrax_s = np.zeros( (nb_frame, 2, 20), float)
    for num_frame in xrange(0, nb_frame):
        max_value, num_delta    = mt.my_max(totalenergy_2m[num_frame, :])
        f0_hz_v[num_frame]        = stock_f0_m[num_frame,num_delta]
        cur_par = dPart
        f_Freq_v                = np.squeeze( F_v[ stock_pos_4m[num_frame,num_delta,num_inharmo, :] ] )
...
def HCalcDescr(f_F0_v, f_DistrPts_m, PartTrax_s):
    ...
    if i_EndFrm == 0:
        ...
        desc_har = dHAR(
        FrameErg        = np.zeros(i_EndFrm-1, float),
        HarmErg            = np.zeros(i_EndFrm-1, float),
        NoiseErg        = np.zeros(i_EndFrm-1, float),
        Noisiness        = np.zeros(i_EndFrm-1, float),
        F0                = np.zeros(i_EndFrm-1, float),
        InHarm            = np.zeros(i_EndFrm-1, float),
        TriStim1        = np.zeros(i_EndFrm-1, float),
        TriStim2        = np.zeros(i_EndFrm-1, float),
        TriStim3        = np.zeros(i_EndFrm-1, float),
        HarmDev            = np.zeros(i_EndFrm-1, float),
        OddEvenRatio    = np.zeros(i_EndFrm-1, float),
        SpecCent        = np.zeros(i_EndFrm-1, float),
        SpecSpread        = np.zeros(i_EndFrm-1, float),
        SpecSkew        = np.zeros(i_EndFrm-1, float),
        SpecKurt        = np.zeros(i_EndFrm-1, float),
        SpecSlope        = np.zeros(i_EndFrm-1, float),
        SpecDecr        = np.zeros(i_EndFrm-1, float),
        SpecRollOff    = np.zeros(i_EndFrm-1, float),
        SpecVar        = np.zeros(i_EndFrm-1, float))
...
def FFTrep(s, Fs):
    ...
    # === pre/post-pad signal
    f_Sig_v = np.concatenate((np.zeros(iHWinSize,float), np.squeeze(f_Sig_v), np.zeros(iHWinSize,float)))
...
def FCalcDescr( f_DistrPts_m, i_SizeX, i_SizeY, f_SupX_v, f_SupY_v ):
    ...
    f_SpecDecr_v    = np.dot(f_Den_v, f_Num_m) /     np.sum(f_DistrPts_m+EPS, axis=0) 
...
def TCalcDescr(trame_s, Fs):
    ...
    m = scipy.io.loadmat(os.path.join(path, 'butter3.mat'))
    ...
    f_AutoCoeffs_v = np.zeros( (config_s.xcorr_nb_coeff, nb_trame), float )
    f_ZcrRate_v = np.zeros( nb_trame, float )
    frame_ind = np.arange(0, N, 1)
    
    
    for n in xrange(0, l_en-N, step):
        #print "Processing frame ", (idx+1)," / ", (nb_trame),"\n"
        i1        = np.round(int(n) + frame_ind )
        f_Frm_v    = np.multiply(np.squeeze(trame_s[i1]), win)
